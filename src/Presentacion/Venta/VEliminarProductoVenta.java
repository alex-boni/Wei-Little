...
public class VEliminarProductoVenta extends JFrame implements IGUI {

	private static final long serialVersionUID = 1L;

	private TCarrito carrito;

	public VEliminarProductoVenta() {
		...
		ok_button.addActionListener((e) -> {
			try {
				...
				if (Integer.valueOf(searchText.getText()) <= 0 || Integer.valueOf(cantidadText.getText()) <= 0)
					...
					Controller.getInstancia().accion(new Context(Evento.ELIMINAR_PP_EN_VENTA_NEGOCIO, carrito));
					this.dispose();
				}
			} catch (Exception ex) {
	...
	public void update(Context context) {
		...
		if (evento == Evento.ELIMINAR_PP_EN_VENTA_VISTA) {
			setVisible(true);
		}
	...
	private void updateVenta(Evento evento, Object datos) {
		...
		if (evento == Evento.RES_PASAR_CARRITO_A_ELIMINAR_OK)
			this.carrito = (TCarrito) datos;

		else if (evento == Evento.RES_PASAR_CARRITO_A_ELIMINAR_KO) {
			mensaje("Error en el traspaso del carrito");
			dispose();
		} else {

			if (evento == Evento.RES_ELIMINAR_PP_EN_VENTA_OK)
				mensaje("Producto eliminado del carrito");

			else if (evento == Evento.RES_ELIMINAR_PP_EN_VENTA_KO)
				mensaje("No se pudo eliminar del carrito comprueba datos");
