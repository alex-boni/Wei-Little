...
public class VistaBajaCliente extends JFrame implements IGUI {
		...
		ok_button.addActionListener((e) -> {
			...
			try {
				if (idTextField.getText().isEmpty())
					...
					Controller.getInstancia().accion(new Context(Evento.BAJA_CLIENTE, id));
					this.dispose();
				}
			} catch (Exception ex) {
	...
	public void update(Context context) {
		if (context.getEvento() == Evento.VBAJA_CLIENTE)
			this.setVisible(true);
		else {
			if (context.getEvento() == Evento.RES_BAJA_CLIENTE_OK)
				JOptionPane.showMessageDialog(null, "Exito: Dado de baja cliente con id: " + (int) context.getDatos());
			else if (context.getEvento() == Evento.RES_BAJA_CLIENTE_KO)
				JOptionPane.showMessageDialog(null, "Error: No se pudo dar de baja al cliente");
			Controller.getInstancia().accion(new Context(Evento.CLIENTE, null));
			this.dispose();
		}
	}
