...
public class VistaBajaProveedor extends JFrame implements IGUI {
	...
	private void rellenarAceptarPanel(JPanel jaceptar) {
		...
		aceptar.addActionListener((e) -> {
			try {
				...
				Controller.getInstancia().accion(new Context(Evento.BAJA_PROVEEDOR_NEGOCIO, id));
				this.dispose();
	...
	public void update(Context c) {
		if (c.getEvento() == Evento.BAJA_PROVEEDOR_VISTA)
			this.setVisible(true);
		else {
			if (c.getEvento() == Evento.RES_BAJA_PROVEEDOR_OK)
				JOptionPane.showMessageDialog(null, "Exito al dar de baja al proveedor id " + ((int) c.getDatos()));
			else if (c.getEvento() == Evento.RES_BAJA_PROVEEDOR_KO)
