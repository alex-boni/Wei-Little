...
public class ModificarEmpleadoCommand implements Command {
	...
	public Context execute(Object transfer) {
		int empleado = FactoriaServicioAplicacion.getInstancia().generaSAEmpleado()
				.modificarEmpleado((TEmpleado) transfer);

		if (empleado == -1) {
			return new Context(Evento.RES_MODIFICAR_EMPLEADO_KO, null);
		} else {
			return new Context(Evento.RES_MODIFICAR_EMPLEADO_OK, empleado);
		}
	}
}