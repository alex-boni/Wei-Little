...
public class AñadirProductoVentaCommand implements Command {
	...
	public Context execute(Object transfer) {
		int result = FactoriaServicioAplicacion.getInstancia().generaSAVenta()
				.insertar_producto_carrito((TCarrito) transfer);

		if (result == 1)
			return new Context(Evento.RES_INSERTAR_PP_EN_VENTA_OK, transfer);
		else
			return new Context(Evento.RES_INSERTAR_PP_EN_VENTA_KO, transfer);
	}
}