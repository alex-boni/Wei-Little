...
public class VistaListarTodoMaquinasPorModelo extends JFrame implements IGUI {

	private static final long serialVersionUID = 1L;

	public VistaListarTodoMaquinasPorModelo() {
		super("[LISTAR MAQUINAS POR MODELO]");
		initGUI();
	}
	...
	private void rellenarmostrarPanel(JPanel jmostrar) {
		...
		JTextField mostrarPorModelo = new JTextField();
		JLabel textoMostrarPorModelo = new JLabel("MOSTRAR POR MODELO");
		JButton mostrarModelo = new JButton("MOSTRAR");

		mostrarPorModelo.setPreferredSize(new Dimension(150, 20));
		textoMostrarPorModelo.setPreferredSize(new Dimension(200, 20));

		mostrarModelo.addActionListener((e) -> {

			try {

				if (mostrarPorModelo.getText().isEmpty()) {
					throw new Exception("Id_modelo no puede ser vacio");
				}

				int id = Integer.parseInt(mostrarPorModelo.getText());

				if (id <= 0) {
					throw new Exception("ID no puede ser negativo ni 0");
				}

				Controller.getInstancia().accion(new Context(Evento.LISTAR_MAQUINAS_X_MODELO_NEGOCIO, (Object) id));
		...
		panel1.add(textoMostrarPorModelo);
		panel1.add(mostrarPorModelo);
	...
	public void update(Context context) {
		if (context.getEvento() == Evento.LISTAR_MAQUINAS_X_MODELO_VISTA) {
			this.setVisible(true);
		} else {
			if (context.getEvento() == Evento.RES_LISTAR_MAQUINAS_X_MODELO_OK) {
			...
			} else if (context.getEvento() == Evento.RES_LISTAR_MAQUINAS_X_MODELO_KO) {

				JOptionPane.showMessageDialog(null, "Error al listar todas las máquinas.");
				Controller.getInstancia().accion(new Context(Evento.LISTAR_MAQUINAS_X_MODELO_VISTA, null));
			}
	...
	class MostrarLista extends JFrame {
		...
		public MostrarLista(Set<TMaquina> tms) {
			super("Lista máquinas por modelo");
			this.tms = tms;
			this.initGUI();
		}
