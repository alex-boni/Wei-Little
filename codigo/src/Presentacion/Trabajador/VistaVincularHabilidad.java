...
public class VistaVincularHabilidad extends JFrame implements IGUI {
	private static final long serialVersionUID = 1L;
	JTextField idhabilidadTextField;
	JTextField idtrabajadorTextField;
	TVinculacionTrabHab tvinculo;

	public VistaVincularHabilidad() {
		super("[VINCULAR HABILIDAD TRABAJADOR]");
		initGUI();
	}
	...
	private void rellenarAceptarPanel(JPanel jaceptar) {
		...
		aceptar.addActionListener((e) -> {
			...
			if (idString1.length() == 0 || idString2.length() == 0) {
				...
				Controller.getInstancia().accion(new Context(Evento.VINCULAR_HABILIDAD_TRABAJADOR, (Object) tvinculo));
				this.dispose();
			}
	...
	public void update(Context context) {
		...
		if (context.getEvento() != Evento.VVINCULAR_HABILIDAD_TRABAJADOR) {
			tvinculo = (TVinculacionTrabHab) context.getDatos();
			if (context.getEvento() == Evento.RES_VINCULAR_HABILIDAD_TRABAJADOR_OK) {
				JOptionPane.showMessageDialog(null, "Exito al vincular habilidad " + tvinculo.get_id_habilidad()
						+ " al trabajador " + tvinculo.get_id_trabajador());
				Controller.getInstancia().accion(new Context(Evento.TRABAJADOR, null));
				this.dispose();
			} else if (context.getEvento() == Evento.RES_VINCULAR_HABILIDAD_TRABAJADOR_KO) {
				JOptionPane.showMessageDialog(null, "Error al vincular al trabajador con la habilidad. ");
				Controller.getInstancia().accion(new Context(Evento.TRABAJADOR, null));
				this.dispose();
			}
